...
#else
	int ret;
	ret = i2c_get_chip_for_busnum(EEPROM_I2C_BUS, EEPROM_I2C_ADDRESS, 1, &dev);
	if (ret) {
...
int Load_config_from_mmc(void)
{
	...
	if (mmc) 
	{
		...
		dev_desc = blk_get_dev("mmc", mmcdevnum);
		if (dev_desc!=NULL) 
		{
...
//
	...
	{//0x01:lvds, 0x02:hdmi, 0x03:RGB
		case 0x03:
			...
			//sprintf(videoprm,"video=mxsfb:dev=lcd,CLAA-WVGA,if=RGB666 galcore.gpuProfiler=1 ");
#else
			//sprintf(videoprm,"video=mxsfb:%dx%dM-%d@%u fbmem=8M ", width, height, AT24c02_eeprom.data.display[3], AT24c02_eeprom.data.display[4]);
		...
		case 0x02:
#ifdef CONFIG_GALCORE_VIVANTE
			sprintf(videoprm,"video=mxcfb0:dev=hdmi,%dx%dM@%u,if=RGB%d,bpp=32 video=mxcfb1:off video=mxcfb2:off fbmem=8M galcore.gpuProfiler=1 ", width, height, AT24c02_eeprom.data.display[4], AT24c02_eeprom.data.display[3]==18?666:24);
#else
			sprintf(videoprm,"video=mxcfb0:dev=hdmi,%dx%dM@%u,if=RGB%d,bpp=32 video=mxcfb1:off video=mxcfb2:off fbmem=8M ", width, height, AT24c02_eeprom.data.display[4], AT24c02_eeprom.data.display[3]==18?666:24);
#endif
